...
namespace WebService.Services.Hubs {
    ...
    public class HubProtocolDH : Hub {
        ...
        /// Иначе - запрос записывается в список запросов с нужный методом синхронизации.
        /// </returns>
        public async Task SyncWeights(string recipient, string hashWeights) {
            SyncRequest request = new SyncRequest {
                Sender = Context.UserIdentifier,
                Recipient = recipient,
                Way = new SyncWay {
            ...
            if (recipientRequest != null) {
                bool successSync = recipientRequest.Way.HashedWeighs == hashWeights;
                // отправка результата синхронизации весов обоим клиентам
                await Clients.User(Context.UserIdentifier).SendAsync("ReceiveSyncWeights", successSync);
                await Clients.User(recipient).SendAsync("ReceiveSyncWeights", successSync);
            }
        }
